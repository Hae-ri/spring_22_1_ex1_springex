...
public class HomeController {
	...
	public String buy_ticket_card(TicketDto ticketDto, Model model) {
		...
		System.out.println("고객 아이디(consumerID): " + ticketDto.getConsumerid());
		System.out.println("카드 결제한 티켓 수(amount): " + ticketDto.getAmount());
		
		dao.buyTicket(ticketDto);
		model.addAttribute("ticketInfo", ticketDto);
		
		int amountCard = ticketDto.getAmount();
		if(amountCard > 4) // rollback
			return "countError";
		
		return "buy_ticket_end"; // commit
	}
